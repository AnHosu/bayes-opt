...
noise <- 0.05
y_train <- demo_objective_function(X_train) + rnorm(5, 0, noise)
...
gp_plot <- gp_1d_plot(
  ...
  X_train = X_train,
  y_train = y_train,
  true_function = demo_objective_function
)
acquisition_plot(
  X_pred = X_pred,
...
Seems like Ackley is actually a 2D test function, so the formula should be changed to x1 and x2.
...
## Functions in n Continuous Dimensions
