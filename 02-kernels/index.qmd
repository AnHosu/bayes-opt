...
Sampled functions from a Gaussian process that utilises the linear kernel exhibit linear behaviour. These sampled functions, appear as straight lines with varying slopes and intercepts.

Indeed, applying a Gaussian process with a linear kernel to data is equivalent to doing Bayesian linear regression.

```{r}
plot_gp(linear_kernel)
```

The linear kernel models a linear relationship between the input vectors. Since this kernel is only able to capture linear patterns in the data, the sampled functions will always be straight lines and will not be able to model other types of variation. However, it can be combined with other kernels to increase its utility.
...
Where $c$ is a constant that determines the shape of the kernel function and $\nu$ is the degree of the polynomial.
...
## Kernels for Discrete and Categorical Data

#### Discrete, Ordered, Inifinite

#### Discrete, Ordered, Finite
...
All the kernels mentioned above deal with continuous data. Discrete kernels quantify the similarity or correlation between data points with discrete features. These kernels are particularly useful in modelling non-continuous data, such as count data, rankings, or time series with irregular intervals.

Discrete kernels still map to a continuous variable and can be applied in Gaussian processes and Bayesian optimisation. However, in contrast to the continuous case where some good general-purpose kernels are available, the discrete kernels tend to be more bespoke.
...
There are two main ways kernels are combined: combining multiple kernels on each feature or using different kernels for different featured.
