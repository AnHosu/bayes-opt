...
Anisotropic kernels that allow for different length scales in different dimensions of the input space. This is in opposition to the isotropic kernels described above, with a single length scale across all dimensions. Intuitively, the anisotropic kernel is stretched or compressed in certain directions compared to its isotropic counterpart. This property can be useful when the input variables have different units or scales.

The anisotropy of a kernel is controlled by several length scale parameters, which specify the characteristic length of variations in the input space along each dimension. The use of anisotropic kernels can improve the predictive accuracy of Gaussian process models when the input variables have different levels of variability or when the correlations between variables vary in different directions. However, anisotropic kernels also increase the complexity of the model because of the added parameters.
...
$$k(\mathbf{x}, \mathbf{x'}) = \sigma^2 \exp\left(-frac{1}{2}(\mathbf{x}-\mathbf{x'})\mathbf{M}(\mathbf{x}-\mathbf{x'})^T)$$
...
#'
#' @return matrix of dimensions (n, m)
anisotropic_rbf_kernel <- function(X1, X2, l = 1.0, sigma = 1.0) {
  if (is.null(dim(X1))) dim(X1) <- c(1, length(X1))
  if (is.null(dim(X2))) dim(X2) <- c(1, length(X2))
  d <- dim(X1)[[2]]
...
This property can be used to create a rotation that effectively reduces the dimensionality of the input.
