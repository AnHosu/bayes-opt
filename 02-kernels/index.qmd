...
Formally, a kernel function $k(\mathbf{x},\mathbf{x'})$ takes two inputs, $\mathbf{x}$ and $\mathbf{x'}$, and returns a real-valued scalar that represents a similarity or dissimilarity measure between them.

A kernel function must be positive semi-definite (PSD) and symmetric. This means that for any set of $n$ input vectors ${\mathbf{x}_1, \ldots, \mathbf{x}n}$, the kernel matrix $\mathbf{\Sigma}$, defined as $\mathbf{\Sigma}{i,j} = k(\mathbf{x}_i, \mathbf{x}_j)$, must be PSD and symmetric. The PSD property ensures that the kernel matrix can be used in positive semidefinite programming formulations, such as in SVMs and kernel ridge regression, while the symmetry property ensures that the kernel measures similarity in a symmetric way between the two objects.
...
Exampels include the Radial Basis Function (RBF) kernel and the Rational Quadratic (RQ) kernel.
