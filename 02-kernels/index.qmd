...
The constant kernel is characterised by its simplicity, as it assumes a constant value, meaning the output is the same for all input points. The constant kernel can be used alone or combined with other kernels to model more complex relationships between data points. In Gaussian process regression, this kernel can capture constant offsets in the function being modelled.
...
#### White Noise Kernel with Heteroskedastic Noise

The white noise kernel is a simple kernel function used in Gaussian processes to model independent noise or measurement errors in the data. The white noise kernel is defined as:
