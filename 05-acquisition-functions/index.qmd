...
kg <- tibble::tibble(x_sim = X_pred) %>%
  dplyr::mutate(
    ...
    # Draw a M = 100 samples of y_sim from the posterior predictive distribution
  ...
  dplyr::mutate(
    ...
    # augment dataset with x_sim, y_sim and get posterior
...
Here is KG for the minimisation case. Notice that the estimate is very rough. The dashed line shows where we would sample next, according to KG.
