...
Mathematically, the acquisition function, $a(\mathbf{x})$, assigns a value to each point in the search space $\mathbf{x} \in \mathcal{X}$. The next point to evaluate, $\mathbf{x}_{t+1}$, is chosen by maximising or minimising the acquisition function, depending on the optimisation task at hand, i.e.
...
$a(\mathbf{x})$ is an acquisition function of a point $\mathbf{x}$ in the search space $\mathcal{X}$. While the search space often contains multiple feature dimensions $\mathcal{X} $ #TODO
...
This function has two minima and two maxima in the range $\mathbf{x} \in [0,1]$, so it will not be too easy to maximise or minimise.
...
#| code-fold: true
#| code-summary: "Show the code"
acquisition_plot <- function(X_pred, ei, gp_plot, xt1, label = "EI", title = "") {
  p1 <- tibble::tibble(
    x = X_pred,
    ei = ei
  ) %>%
...
acquisition_plot(X_pred, ei, gp_plot, xt1, "EI", "Expected Improvement (Minimisation)")
```

The next sampling point, $\mathbf{x}_{t+1}$, is the one that maximises the acquisition function, here EI. In this case, this is close to the right edge, where there is a high mean prediction but also high uncertainty, so it satisfies our need for both exploration and exploitation.
...
acquisition_plot(X_pred, pi, gp_plot, xt1, "PI", "Probability of Improvement (Minimisation)")
```
...
```{r}
ucb <- confidence_bound(mu, sigma, kappa = 2, "min")
...
$$\mathbf{x}_{t+1} \in \arg\min\limits_{\mathbf{x} \in \mathcal{X}}(a_{ts}(\mathbf{x}))$$
...
```{r}
post <- rlang::exec(posterior, rbf_kernel, X_pred, X_train, y_train, !!!post_pred$parameters)
ts <- as.vector(rmvnorm(1, post$mu, post$sigma))
xt1 <- X_pred[which.min(ts)]
acquisition_plot(X_pred, ts, gp_plot, xt1, "Sample", "Thompson Sampling (Minimisation)")
```
```{r}
xt1 <- X_pred[which.max(ts)]
acquisition_plot(X_pred, ts, gp_plot, xt1, "Sample", "Thompson Sampling (Maximisation)")
```
...
In most real world cases, we are risk averse and would opt to select a point that we have already tested, i.e. $\mathbf{x}^* \in \mathbf{X}$. Imagine, however, that we were not risk averse and just wanted to report the point with the best expected outcome given the data so far, then we might recommend

$$\mathbf{x}^* = \arg\min\limits_{\mathbf{x} \in \mathcal{X}}\mu(\mathbf{x})$$

We also define $\mu^* = \mu(\mathbf{x}^*)$.

At this point, imagine that we are suddenly allowed to test just one more point. Given that we have decided to recommend the point that optimises the posterior mean at the end of our experimental sequences, the next point, $\mathbf{x}_{t+1}$, we choose to evaluate in the the objective function should be the point that maximises the increase in the optimum posterior mean.
...
This is the definition of the Knowledge Gradient for a minimisation problem.
...
To calculate KG in practice, we need a way to estimate $\mu^*_{t+1}$ as a function of existing observations and we need a way to integrate over $p(y|\mathcal{D})$. For a proposed point, $\mathbf{x}_{sim}$, the solution to the former is to draw a sample $y_{sim} \sim p(y|\mathbf{x}_{sim},\mathcal{D})$, create an augmented dataset $\mathcal{D}^+ = \{\{\mathbf{X},\mathbf{x}_{sim}\}, \{\mathbf{y},y_{sim}\}\}$, condition the surrogate model on the augmented dataset, and compute the optimum of the new posterior mean.

```{r}
# for i in J
#   sample a function from the posterior predictive distribution
#   find the optimum of the sampled function
#   store the difference between the optimum value and each point in the posterior mean
# 
mu_min <- min(mu)
mu_max <- max(mu)
kg <- tibble::tibble(x_sim = X_pred) %>%
  dplyr::mutate(
    post = purrr::map(x_sim, function(x) {
      rlang::exec(
        posterior,
        kernel = rbf_kernel,
        X_pred = x,
  ...
  dplyr::mutate(
    # augment dataset with x_sim, y_sim and get posterior for sim
    ...
    mu_diff_max = mu_max_sum - mu_max
  ) %>%
  dplyr::group_by(x_sim) %>%
  dplyr::summarise(kg = mean(mu_diff)) %>%
  dplyr::pull(kg)

acquisition_plot(X_pred, kg, gp_plot, X_pred[which.max(kg)])
```
