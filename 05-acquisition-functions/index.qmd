...
```{r}
ts <- as.vector(rmvnorm(1, mu, diag(sigma))) # TODO: I think this should be the full posterior sigma instead
...
# 
mu_min <- min(mu)
kg <- tidyr::expand_grid(x_sim = X_pred, j = 1:50) %>%
  dplyr::mutate(
    post = purrr::map(x_sim, function(x) {
      rlang::exec(posterior, rbf_kernel, x, X_train, y_train, !!!post_pred$parameters)
    }),
    y_sim = purrr::map_dbl(post, function(p) {
      rlang::exec(rmvnorm, n = 1, mu = p$mu, sigma = p$sigma)
    }),
    # augment dataset with x_sim, y_sim and get posterior for sim
    post_sim = purrr::map2(x_sim, y_sim, function(xs, ys) {
      rlang::exec(posterior, rbf_kernel, X_pred, rbind(X_train, xs), rbind(y_train, ys), !!!post_pred$parameters)
    }),
    mu_max_sim = purrr::map_dbl(post_sim, ~ min(.x$mu)),
    mu_diff = mu_min - mu_max_sim
  ) %>%
