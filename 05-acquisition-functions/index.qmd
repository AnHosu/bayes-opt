...
For the demonstration of acquisition functions, we are going to need a toy problem. We will use a simple objective function. For this demonstration, the observations will not be noisy.
...
The model will receive four training points. We perform Gaussian process regression and condition the Gaussian process on a grid of $\mathbf{x} \in \mathcal{X}$. The $\mu$ and $\sigma$ of the posterior predictive distribution are needed to calculate some acquisition functions.

```{r}
X_train <- matrix(c(0.1, 0.4, 0.7, 0.75), 4, 1)
...
acquisition_plot(X_pred, ei, gp_plot, label = "EI", title = "Expected Improvement")
```
...
```{r}
probability_of_improvement <- function(mu, sigma, y_best, xi = 0.01, task = "min") {
  if (task == "min") imp <- y_best - mu - xi
  if (task == "max") imp <- mu - y_best - xi
  pnorm(imp / sigma)
}
...
acquisition_plot(X_pred, pi, gp_plot, label = "PI", title = "Probability of Improvement")
```
...
```{r}
cb <- confidence_bound(mu, sigma, kappa = 2)
acquisition_plot(X_pred, cb, gp_plot, label = "PI", title = "Lower Confidence Bound")
```
