...
For the demonstration of acquisition functions, we need a toy problem. We will use a simple objective function with noise and a single dimension.
...
The surrogate model will receive five training points and it should be able to return $\mu(\mathbf{x})$ and $\sigma(\mathbf{x})$ for all points in the search space, which we will approximate with a grid.
...
gp_plot <- gp_1d_plot(mu, sigma, X_pred, X_train, y_train, objective_function)
...
A single deterministic model will not work as a surrogate for Bayesian optimisation, as it does not offer a measure of uncertainty. Fortunately there is a way for us to use our favourite machine learning model for Bayesian optimisation. If we collect an ensemble of models, we can use the mean and standard deviation of the predictions in place of $\mu(\mathbf{x})$ and $\sigma(\mathbf{x})$.

There are a few ways in which we could build an ensemble, and they all work as long as we can obtain a measure of uncertainty. For instance, if we had a lot of training data, we could do bagging and train several models on different subsets of the training data and use those as an ensemble. We could also use a stack of different models as an ensemble.
...
As a demonstration of an ensemble used as a surrogate model in the case of our running example, we will fit neural networks. We fit a simple two-layer NN with 5 nodes in each. Rather than fitting different models, we use 20 different initialisations of the same architecture to create an ensemble.
...
ens_plot <- gp_1d_plot(preds$mean, preds$sd, X_pred, X_train, y_train, objective_function)
acquisition_plot(
  X_pred,
  ei_nn_ens,
  ens_plot,
  X_pred[which.max(ei_nn_ens)],
  label = "EI",
...
Not that an ensemble of NNs is not the right choice 
...
bnn_plot <- gp_1d_plot(y_pred$mean, y_pred$sd, X_pred, X_train, y_train, objective_function)
...
y_pred <- rstan::summary(fit, pars = "y_pred")$summary %>%
  tibble::as_tibble()
# Calculate EI and plot
ei_besp <- expected_improvement(y_pred$mean, y_pred$sd, min(y_train))
besp_plot <- gp_1d_plot(y_pred$mean, y_pred$sd, X_pred, X_train, y_train, objective_function)
acquisition_plot(X_pred, ei_besp, besp_plot, X_pred[which.max(ei_besp)], label = "EI", title = "Bespoke Model Surrogate")
```
...
To implement a Student's t Process, we need three things: an expression for the negative log likelihood so we can optimise hyperparameters of the process, an expression for the posterior predictive distribution parameters so we can generate predictions, and a function for sampling from the multivariate Student's t distribution.
...
#' @param X_train matrix (n, d) of training points
#' @param y_train column vector (n, d) of training observations
#' @param nu scalar, prior degrees of freedom
#' @param noise scalar of observation noise
#' @param ... named parameters for the kernel function
#'
#' @return list of mean (mu), covariance (sigma), and degrees of freedom (nu)
#' for the Student's t distribution
posterior_t <- function(kernel, X_pred, X_train, y_train, nu, noise = 1e-8, ...) {
  if (is.null(dim(X_pred))) dim(X_pred) <- c(length(X_pred), 1)
  if (is.null(dim(X_train))) dim(X_train) <- c(length(X_train), 1)
  if (is.null(dim(y_train))) dim(y_train) <- c(length(y_train), 1)
...
$$\text{NLL}(y|X) = -\log\left(\frac{\Gamma\left(\frac{\nu + n}{2}\right)}{\Gamma\left(\frac{\nu}{2}\right)(\nu \pi)^{\frac{n}{2}}\sqrt{\lvert K + \sigma^2I \rvert}\left(1 + \frac{1}{\nu} y^T(K + \sigma^2I)^{-1}y\right)^{\frac{\nu + n}{2}}}\right)$$

Implementation inspired by algo from Rasmussen 2006 ch 2.
...
```{r}
dlst <- function(x, nu = 1, mu = 0, sigma = 1) {
  1 / sigma * dt((x - mu) / sigma, nu)
}
plst <- function(q, nu = 1, mu = 0, sigma = 1)  pt((q - mu) / sigma, nu)
qlst <- function(p, nu = 1, mu = 0, sigma = 1)  qt(p, nu) * sigma + mu
...
expected_improvement_t <- function(mu,
  ...
  ei <- imp * plst(Z, nu = nu) + scaled_sigma * dlst(Z, nu = nu)
  ei[sigma == 0.0] <- 0.0
  ei
}
```
...
```{r}
tp_1d_plot <- function(mu, sigma, nu, X_pred, X_train, y_train, true_function) {
  tibble::tibble(
    x = X_pred,
    mu = mu,
    sigma = sigma,
    upper = qt(0.945, nu, TRUE, FALSE) * sigma + mu,
    lower = qt(0.055, nu, TRUE, FALSE) * sigma + mu,
    f = true_function(X_pred)
  ) %>%
    ggplot(aes(x = x)) +
...
tp <- tpr(rbf_kernel, X_train, y_train, nu = 5, noise = 0.1, l = 1, sigma_f = 1)
post_pred <- tp(X_pred)
mu <- post_pred$mu
sigma <- post_pred$sigma
nu <- post_pred$nu
ei <- expected_improvement_t(mu, sigma, nu, min(y_train))
tp_plot <- tp_1d_plot(mu, sigma, nu, X_pred, X_train, y_train, objective_function)
acquisition_plot(X_pred, ei, tp_plot, xt1 = X_pred[which.max(ei)], label = "EI", title = "Student's t Process Surrogate")
```
