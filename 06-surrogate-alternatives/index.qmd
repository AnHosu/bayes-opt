...
Since the focus of this post is the application of surrogate models, we will just quickly review the expressions for these components. This does mean that we will be making some tacit assumptions. For a comprehensive treatment of TPs, see [@Shah2014] and [@Tang2017].
...
The log likelihood [@Rasmussen:2006] [@Shah2014]

$$\begin{aligned} \log p(\mathbf{y}_t \mid \mathbf{X}_t, \theta) =\,& \log\Gamma\left(\frac{\nu + d}{2}\right) \\ &- \log\Gamma\left(\frac{\nu}{2}\right) \\ &- \frac{1}{2}\log\det (\mathbf{\Sigma}_{tt} + \sigma_{\epsilon}^2 \mathbf{I}) \\ &- \frac{1}{2}(\nu+d)\log\left(1+\frac{\mathbf{y}_t^T (\mathbf{\Sigma}_{tt} + \sigma_{\epsilon}^2 \mathbf{I})^{-1} \mathbf{y}_t}{\nu-2}\right) \\ &- \frac{1}{2}d \log((\nu-2)\pi) \end{aligned}$$

Where $d$ is the number of dimensions of the distribution
...
nll_t <- function(kernel, X_train, y_train, nu, noise) {
  function(params) {
    ...
    a <- backsolve(r = L, x = forwardsolve(l = t(L), x = y_train))
    beta <- t(y_train) %*% a
    (d * log((nu - 2) * pi) + logdet + (nu + d) * log(1 + beta / (nu - 2))) %>%
      multiply_by(0.5) %>%
