...
```{r}
View(rstan::summary(fit)$summary)
...
rmvst <- function(n = 1, nu, mu, sigma, epsilon = 1e-6) {
  ...
  # TODO: something is not quite right. The tails are not long enough...
  t_samples <- sweep(standard_normal_samples %*% cholesky, 2, mu, FUN = `+`)
  t_samples <- sweep(t_samples, 1, chi_samples, FUN = `/`)
  t_samples
}
