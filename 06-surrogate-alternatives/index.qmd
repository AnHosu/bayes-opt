...
```{r}
objective_function <- function(x) sin(12 * x) * x + 0.5 * x^2
```

We assume that we want to minimise this function. There are two minima in the search space $\mathcal{X} = [0,1]$, so it will not be too easy to minimise.
...
Where $\nu_{prior}$ is the prior degrees of freedom, $d$ is the number of dimensions of the distribution, and $\mathbf{\Sigma}_{tt}$ is the scaled covariance matrix between training points [@Rasmussen:2006] [@Tracey2018].
