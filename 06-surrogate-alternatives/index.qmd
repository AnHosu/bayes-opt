...
TPs are a little bit different. Specifically, TPs are parametrised by a shape parameter, $\mathbf{\Sigma}$, which is a scaled covariance matrix:
...
##### SIGMA SCALING, see [@Tracey] eq 27

Diagonal noise, see [@Shah2014] 4.6. We add noise to the diagonal before scaling, as if we were adding a white noise kernel with a fixed parameter. This is not equivalent to the GP way of accounting for the noise, since we have the scaling in TPs. But for infinite data or infinite nu prior, the two are equivalent.
...
##### Change to K and add noise term for K_tt
...
Where $\mathbf{\Sigma}_{pp}$ is the covariance matrix between prediction points, $n_t$ is the number of training observations, and where we once again assume $\mathbf{\mu}_t = \mathbf{0}$.
...
The covariance matrix, $\mathbf{\Sigma}$, is calculated using a kernel function with parameters, $\theta$. Analogous to Gaussian process regression, we find good values for these parameters by minimising the negative log likelihood in what is called Student's t process regression.
...
Where $\nu$ is the prior degrees of freedom, $d$ is the number of dimensions of the distribution, $\mathbf{\Sigma}_{tt}$ is the covariance matrix between training points,  [@Rasmussen:2006] [@Shah2014]
...
We then utilise TP with these parameters for calculating the acquisition function.
...
When using a TP surrogate model, EI can be calculated using the formula 
