...
Since the focus of this post is the application of surrogate models, we will just quickly review the expressions for these components. This does mean that we will be making some tacit assumptions. For a comprehensive treatment of TPs, see [@Shah2014], [@Tang2017], and [@Tracey2018].
...
Where $k$ is a [kernel function](.../kernels-r).

TPs are a little bit different. Specifically, TPs are parametrised by a shape parameter, $\mathbf{\Sigma}$, which is a scaled covariance matrix [@Tracey]:
...
$$\mathbf{\mu}_{p|t} = \mathbf{\mu}_p + \mathbf{K}_{tp}^T \mathbf{K}_{tt}^{-1} (\mathbf{y}_t - \mathbf{\mu}_t)$$


Where $\mathbf{K}_{tp}$ is the covariance matrix between training and prediction points and $\mathbf{K}_{tt}$ is the covariance matrix between training points.

We often assume that $\mathbf{\mu}_t = \mathbf{0}$, so the equation will show up as 

$$\mathbf{\mu}_{p|t} = \mathbf{K}_{tp} \mathbf{K}_{tt}^{-1} \mathbf{y}_t$$
...
Where $\mathbf{K}_{pp}$ is the covariance matrix between prediction points, $n_t$ is the number of training observations, and where we once again assume $\mathbf{\mu}_t = \mathbf{0}$.
...
The log likelihood likelihood function for at TP is:

$$\begin{aligned} \log p(\mathbf{y}_t \mid \mathbf{X}_t, \theta) =\,& \log\Gamma\left(\frac{\nu + d}{2}\right) \\ &- \log\Gamma\left(\frac{\nu}{2}\right) \\ &- \frac{1}{2}\log\det (\mathbf{\Sigma}_{tt}) \\ &- \frac{1}{2}(\nu+d)\log\left(1+\frac{\mathbf{y}_t^T (\mathbf{\Sigma}_{tt})^{-1} \mathbf{y}_t}{\nu}\right) \\ &- \frac{1}{2}d \log(\nu\pi) \end{aligned}$$

Where $\nu$ is the prior degrees of freedom, $d$ is the number of dimensions of the distribution, $\mathbf{\Sigma}_{tt}$ is the scaled covariance matrix between training points [@Rasmussen:2006] [@Tracey2018].

Despite being in the logarithmic domain, the log likelihood is still numerically challenged. To create a robust implementation we build and algorithm inspired by the one presented for GPs in chapter 2 of [@Rasmussen:2006].
...
Now we are ready to apply our Student's t process to our running example. 
...
y_extra_train <- objective_function(X_extra_train) + rnorm(n_extra, 0, noise_extra) / sqrt(5 / rchisq(n_extra, 5))

# kernel_nll <- nll_t(rbf_kernel, X_train = X_extra_train, y_train = y_extra_train, nu = 5, noise = 02)
# 
# opt_params <- purrr::map(1:1000, function(i) {
#   optim(par = rexp(2, 1), fn = kernel_nll, method = "BFGS")$par
# })
# a <- opt_params %>% do.call(rbind, .)
# nll_vals <- purrr::map_dbl(opt_params, kernel_nll)
# opt_param <- opt_params[[which.min(nll_vals)]]
# 
# L <- rlang::exec(rbf_kernel, X1 = X_extra_train, X2 = X_extra_train, l = 0.0254096, sigma_f = 0.27813) %>%
#       add(noise**2 * diag(n_extra)) %>%
#       multiply_by(nu / (nu - 2)) %>%
#       chol()
# logdet <- sum(log(diag(L)))
# a <- backsolve(r = L, x = forwardsolve(l = t(L), x = y_extra_train))
# beta <- t(y_extra_train) %*% a
# d <- 1
# nu <- 5
# (nu + d) * log(1 + beta / nu) + logdet
# 
# (d * log(nu * pi) + logdet + (nu + d) * log(1 + beta / nu)) %>%
#   multiply_by(0.5) %>%
#   add(lgamma(nu / 2) - lgamma((nu + d) / 2))
