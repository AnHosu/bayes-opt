...
model <- rstan::stan_gamm4(formula, 
                    data = list(x = x, y = y), 
                    family = gaussian(),
                    prior = brms::set_prior("bnn(1, prior = normal(0, 1))"), 
                    chains = 4, iter = 2000)



model <- brms::brm(y ~ bnn(x, hidden = c(10, "relu")), 
...
x <- X_train # matrix(seq(0, 1, 0.05))
# Prepare the data
data <- list(N = dim(X_train)[[1]], K = dim(X_train)[[2]], X = X_train, y = as.vector(y_train), M = dim(X_pred)[[1]], X_pred = X_pred)

# Fit the model using MCMC sampling
fit <- rstan::sampling(model, data = data, chains = 4, cores = 4, iter = 2000)
