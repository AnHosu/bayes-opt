...
posterior_t <- function(kernel, X_pred, X_train, y_train, noise = 1e-8, ...) {
  if (is.null(dim(X_pred))) dim(X_pred) <- c(length(X_pred), 1)
  if (is.null(dim(X_train))) dim(X_train) <- c(length(X_train), 1)
  if (is.null(dim(y_train))) dim(y_train) <- c(length(y_train), 1)
  ...
  scale <- ((t(y_train) %*% K_inv) %*% y_train - 2) / (n_train - 2)
  sigma <- K_tilde * as.vector(scale)
  list(mu = mu, sigma = sigma, nu = n_train)
}
```
...
tpr <- function(kernel, X_train, y_train, nu = 3, noise = 1e-8, ...) {
  ...
  param <- list(...)
  opt <- optim(par = rep(1, length(param)), fn = kernel_nll) # TODO needs restarts with random inits
  opt_param <- opt$par
  function(X_pred) {
    post <- rlang::exec(
      posterior_t,
...
```{r}
tp <- tpr(rbf_kernel, X_train, y_train, nu = 3, noise = 0.1, l = 1, sigma_f = 1)
post_pred <- tp(X_pred)
mu <- post_pred$mu
sigma <- post_pred$Sigma
nu <- post_pred$nu
samples <- rmvst(n = 10, mu, sigma, nu)
```

```{r}
samples <- rmvst(n = 1e3, mu, sigma, nu)
tibble::tibble(
  x = X_pred,
  mu = mu,
  upper = apply(samples, 2, quantile, probs = 0.945),
  lower = apply(samples, 2, quantile, probs = 0.055)
) %>%
  ggplot() +
...
```{r}
opt <- optim(par = c(1,1), fn = tnl, method = "BFGS")
```
