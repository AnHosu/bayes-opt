...
```{r}
preds <- purrr::map(1:100, function(i) {
  m <- lm(
    y ~ .,
    data = dplyr::slice_sample(train, n = 3)
  )
  predict(m, pred)
}) %>%
  do.call(cbind, .) %>%
  list(mean = apply(., 1, mean), sd = apply(., 1, sd))
ei_lm_ens <- expected_improvement(preds$mean, preds$sd, min(y_train))
lm_ens_plot <- uncertainty_plot(preds$mean, preds$sd, X_train, y_train)
acquisition_plot(X_pred, ei_lm_ens, lm_ens_plot, X_pred[which.max(ei_nn_ens)], label = "EI", title = "Linear Model Ensemble Surrogate")
```
...
ps <- purrr::map(1:10, function(i) {
  preds <- purrr::map(1:20, function(i) {
    ...
    do.call(cbind, .) %>%
    list(mean = apply(., 1, mean), sd = apply(., 1, sd))
  ei_ens <- expected_improvement(preds$mean, preds$sd, min(bo_train$y))
  ens_plot <- uncertainty_plot(preds$mean, preds$sd, bo_train$x, bo_train$y)
  xt1 <- X_pred[which.max(ei_ens)]
  bo_train <<- dplyr::bind_rows(
    bo_train,
    tibble::tibble(x = xt1, y = objective_function(xt1) + rnorm(1, 0, 0.1))
  )
  acquisition_plot(X_pred, ei_ens, ens_plot, xt1, label = "EI", title = "Linear Model Ensemble Surrogate")
})
...
```{r}
# Create a Stan model using the neural network architecture
model <- rstan::stan_model("bnn.stan")

# Prepare the data
# X_train <- matrix(seq(0, 1, 0.2))
# y_train <- objective_function(X_train)
data <- list(N = dim(X_train)[[1]], K = dim(X_train)[[2]], X = X_train, y = as.vector(y_train), M = dim(X_pred)[[1]], X_pred = X_pred, sigma = 0.05)

# Fit the model using MCMC sampling

fit <- rstan::sampling(model, data = data, chains = 4, cores = 4, iter = 2000, seed = seed)
...
tibble::tibble(
    mu = y_pred$mean,
    uncertainty = 1.96*sqrt(y_pred$sd),
...
tibble::as_tibble(summ) %>%
  dplyr::mutate(param = rownames(summ)) %>% 
  View()
