...
tpr <- function(kernel, X_train, y_train, nu = 3, noise = 1e-8, ...) {
  ...
  #  minima.
  opt_params <- purrr::map(1:500, function(i) {
    optim(par = rexp(length(param), 1), fn = kernel_nll)$par
  })
...
n_extra <- 8
noise_extra <- 0.2
X_extra_train <- matrix(seq(0, 1, length.out = n_extra), n_extra, 1)
y_extra_train <- objective_function(X_extra_train) + rnorm(n_extra, 0, noise_extra) / sqrt(6 / rchisq(n_extra, 6))
...
opt_params <- purrr::map(1:1000, function(i) {
  optim(par = rexp(2, 1), fn = kernel_nll)$par
})
