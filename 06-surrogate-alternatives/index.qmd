...
For this example, we will use a NN with a single hidden layer before the output layer. The NN has weights and biases that need priors. We will just use a wide normal prior

$$\{\mathbf{w}_{hidden}, \mathbf{b}_{hidden}, \mathbf{x}_{output}, b_{output}\} \sim \mathcal{N}(0, 4)$$
...
```{r}
pairs(fit, pars = c("w1"))
hist(extraDistr::rinvgamma(1e5, 6, 4), breaks = 1e3)
```
