...
```{r}
matern_kernel <- function(x, x_prime, nu, lengthscale) {
  distance <- dist(x, x_prime)  # Calculate Euclidean distance
  common_term <- sqrt(2 * nu) * distance / lengthscale
  (2^(1 - nu) / gamma(nu)) * (common_term^nu) * besselK(common_term, nu)
}
```
