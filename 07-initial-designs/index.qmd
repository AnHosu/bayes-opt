...
def i2_generator(dim, n):
    ...
    for i in range(n):
        ...
        for j in range(1, dim):
            v[j] = v[j-1] ^ (v[j-1] >> 1) ^ a[j]
            x[i, j] = v[j] / 2**m[j]
    return x

def sobol_sequence(dim, n, skip=0):
    x = i2_generator(dim, n+skip)
    return x[skip:]
...
```{r}
i2_generator <- function(dim, n) {
  v <- rep(0, times = dim)
  a <- rep(0L, times = dim)
  m <- rep(0L, times = dim)
  m[1] <- 1L
  for (i in seq_len(dim - 1) + 1) {
    a[i] <- a[i - 1]^(bitwShiftR(a[i - 1], 1))
    m[i] <- bitwShiftL(m[i - 1], 1)
  }
  x <- matrix(0, n, dim)
  for (i in seq_len(n)) {
    x[i, 0] <- 0
    for (j in seq_len(dim - 1) + 1) {
      v[j] <- v[j - 1]^(bitwShiftR(v[j - 1], 1))^a[j]
      x[i, j] <- v[j] / 2**m[j]
    }
  }
  return(x)
...
i2_generator <- function(dim, n) {
  ...
  m[1] <- 1L
  for (i in 2:dim) {
    a[i] <- bitwXor(a[i-1], bitwShiftR(a[i-1], 1))
    m[i] <- bitwShiftL(1, i-1)
  }
  x <- matrix(0, n, dim)
  for (i in 1:n) {
    x[i,1] <- 0
    for (j in 2:dim) {
      v[j] <- bitwXor(v[j-1], bitwXor(bitwShiftR(v[j-1], 1), a[j]))
      x[i,j] <- v[j] / bitwShiftL(1, m[j])
    }
  }
  x
}

sobol_sequence <- function(dim, n, skip=0) {
  x <- i2_generator(dim, n+skip)
  x[(skip+1):n,]
}
...
quasi_random_sequence <- function(n, dim=2, bases=c(2,3)) {
  seq <- matrix(0, n, dim)
  for (d in 1:dim) {
    seq[,d] <- van_der_corput(n, bases[d])
  }
  seq
}
