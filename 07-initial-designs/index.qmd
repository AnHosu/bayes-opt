...
We can turn a continuous range into a discrete range by cutting it into intervals.

Imagine that we had a discrete feature with five possible values. To include it in the initial design, we generate a dimension for this feature, just as we would for a continuous feature. We then define five intervals along the unit range and let each interval correspond to a step in the discrete feature.

```{r}
design <- latin_hypercube_sampling(n, d)
design[,2] <- cut(design[,2], breaks = 5)
